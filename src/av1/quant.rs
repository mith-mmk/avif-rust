...
// AV1 uses dedicated high-bit-depth quantizer tables; they are not a simple
// power-of-two scaling of the 8-bit tables.
const DC_QLOOKUP_10: [i32; 256] = [
...
fn dc_q(qindex: i32, bit_depth: u8) -> i32 {
    ...
    match bit_depth {
        8 => DC_QLOOKUP_8[index],
        10 => DC_QLOOKUP_10[index],
        12 => DC_QLOOKUP_12[index],
        _ => unreachable!("bit depth is validated by QuantState"),
    }
}
...
fn ac_q(qindex: i32, bit_depth: u8) -> i32 {
    let index = clip_qindex(qindex);
    match bit_depth {
        8 => AC_QLOOKUP_8[index],
        10 => AC_QLOOKUP_10[index],
        12 => AC_QLOOKUP_12[index],
        _ => unreachable!("bit depth is validated by QuantState"),
    }
}
...
mod tests {
    ...
    #[test]
    fn qlookup_uses_dedicated_high_bit_depth_tables() {
        assert_eq!(dc_q(255, 10), 5347);
        assert_eq!(ac_q(255, 12), 29247);
    }
