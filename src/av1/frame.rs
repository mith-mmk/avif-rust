...
mod tests {
    use super::*;
    use crate::av1::parse_sequence_header;
    use crate::container::parse_avif;
    use crate::obu::{ObuType, find_obu_payload};

    fn sample_headers() -> (SequenceHeader, Vec<u8>) {
        ...
        (
            parse_sequence_header(sequence_payload).unwrap(),
            frame_payload.to_vec(),
        )
    }

    #[test]
    fn parses_sample_frame_header_basics() {
        let (sequence, frame_payload) = sample_headers();
