...
#[test]
#[ignore]
fn debug_black_fixture_first_block() {
    let data = std::fs::read("test_data/images/BlackLossless.avif").unwrap();
    let info = parse_avif(&data).unwrap();
    let config_payload = info.av1_config.as_ref().unwrap();
    let config = parse_av1_config(config_payload).unwrap();
    ...
    let sequence = parse_sequence_header(sequence_payload).unwrap();
    let frame = parse_frame_header(frame_payload, &sequence).unwrap();
    let group = parse_tile_group(frame_payload, frame.uncompressed_header_bits, &frame.tile_info).unwrap();
    let plan = build_still_decode_plan(&sequence, &frame, &group).unwrap();
    let modes = avif_rust::av1::probe_tile_block_modes(&info.primary_item_payload, &group, &sequence, &frame, &plan).unwrap();
    let residuals = avif_rust::av1::probe_first_block_residuals(&info.primary_item_payload, &group, &sequence, &frame, &plan).unwrap();
    println!("config={config:?} sequence={sequence:?} frame={frame:?}");
    println!("mode={:?}", modes.first());
    println!("residual={:?}", residuals.first());
    let mut buffers = alloc_frame_buffers(&plan).unwrap();
    let blocks = avif_rust::av1::decode_luma_root_blocks(
    ...
    .unwrap();
    println!("blocks={:?} first_planes={:?}", blocks.iter().map(|b| (b.x, b.y, b.block_size)).collect::<Vec<_>>(), buffers.planes.iter().map(|p| &p.samples[..8]).collect::<Vec<_>>());
    let full = avif_rust::decode_frame_bytes(&data).unwrap();
    println!("full first_planes={:?}", full.buffers.planes.iter().map(|p| &p.samples[..8]).collect::<Vec<_>>());
}
